...
class TransactionManager(object):
    ...
    def patch(self):
        ...
        if not self._patched_var:
            ...
            #transaction.rollback_unless_managed = self._patched(transaction.rollback_unless_managed, False, unless_managed=True)
            transaction.commit = self._patched(transaction.commit, True)
            #transaction.commit_unless_managed = self._patched(transaction.commit_unless_managed, True, unless_managed=True)
